...
namespace EasyFramework.Editor
{
    public class TerrainTextureExport : EditorWindow
    {
        ...
        private void ExportDetailMap()
        {
            ...
            string detailTexturePath = Path.Combine(absolutePath, $"DetailData.raw");
            var file = File.Open(detailTexturePath, FileMode.Create);
            var writer = new BinaryWriter(file);

            for (int i = 0; i < detailLayerNum; i++)
            {
                byte layerMaxDensity = 0;
                var detailMapData = terrainData.GetDetailLayer(0, 0, terrainData.detailWidth, terrainData.detailHeight, i);
                var Width = detailMapData.GetLength(0);
                var height = detailMapData.GetLength(1);
                detailDensityData[i] = new byte[Width * height];

                for (int j = 0; j < height; j++)
                {
                    for (int k = 0; k < Width; k++)
                    {
                        var density = (byte)detailMapData[j, k];
                        detailDensityData[i][j * Width + k] = density;
                        layerMaxDensity = (layerMaxDensity >= density) ? layerMaxDensity : density;
                        totalMaxDensity = (totalMaxDensity >= density) ? totalMaxDensity : density;
                    }
                }
                layerMaxDensityList[i] = layerMaxDensity;
                writer.Write(detailDensityData[i]);
            }
            ...
            _easyGrassData.MaxCountPerPatch = 4096;
            _easyGrassData.CullDistance = 100;
            _easyGrassData.DetailDensity = 1.0f;
            _easyGrassData.CellSize = new Vector2Int(32, 32);
